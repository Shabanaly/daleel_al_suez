...
import {
    User, Shield, ShieldAlert, ShieldCheck,
...
import { updateUserRole } from '@/actions/admin/users.actions'
import { getUserStats, getUserLogs, type Profile, type UserStats, type AuditLog } from '@/services/admin/users.service'

// Helper function to get action icon and color
function getLogActionStyle(action: string) {
    const styles: Record<string, { icon: any, color: string, bg: string, label: string }> = {
...
export default function UsersClientPage({ initialUsers, currentUserRole }: { initialUsers: Profile[], currentUserRole: string | null }) {
    ...
    const fetchUserDetails = async (userId: string) => {
        ...
        try {
            const { getUserDetails } = await import('@/actions/admin/users.actions')
                                            ...
                                            <td colSpan={4} className="p-0">
                                                <div className="p-4 md:p-6 border-b border-slate-800 animate-in slide-in-from-top-2">
                                                    ...
                                                    <div className="md:hidden flex items-center gap-2 text-xs text-slate-500 mb-4 px-1">
                                                        <Calendar size={14} />
                                                        <span>تاريخ الانضمام: {format(new Date(user.created_at), 'dd MMM yyyy', { locale: ar })}</span>
                                                    </div>
                                                                                            ...
                                                                                            <div className="flex-1 min-w-0">
                                                                                                <div className="flex items-start justify-between gap-2 mb-1">
                                                                                                    ...
                                                                                                    <span className="text-[10px] text-slate-500 shrink-0">
                                                                                                        {formatLogTimestamp(new Date(log.created_at))}
                                                                                                    </span>
                                                                                                </div>

    ...
    return (
        <footer className="bg-slate-900 text-slate-200 py-12">
            <div className="container mx-auto px-4 grid grid-cols-1 md:grid-cols-4 gap-8">
                <div>
                    <h3 className="text-xl font-bold text-white mb-4">{settings?.site_name || "دليل السويس"}</h3>
                    <p className="text-sm text-slate-400">
                        {settings?.site_description || "الدليل الشامل لكل ما تحتاجة في مدينة السويس. مطاعم، كافيهات، خدمات، والمزيد."}
                    </p>
                </div>
                        ...
                        {settings?.social_links && Array.isArray(settings.social_links) ? (
                            ...
                            <>
                                {settings?.social_facebook && (
                                    <a href={settings.social_facebook} target="_blank" rel="noopener noreferrer" className="p-2 bg-slate-800 rounded-full hover:bg-primary hover:text-white transition-colors">
                                        <Facebook size={20} />
                                    </a>
                                )}
                                {settings?.social_instagram && (
                                    <a href={settings.social_instagram} target="_blank" rel="noopener noreferrer" className="p-2 bg-slate-800 rounded-full hover:bg-primary hover:text-white transition-colors">
                                        <Instagram size={20} />
                                    </a>
                                )}
                                {settings?.social_twitter && (
                                    <a href={settings.social_twitter} target="_blank" rel="noopener noreferrer" className="p-2 bg-slate-800 rounded-full hover:bg-primary hover:text-white transition-colors">
                                        <Twitter size={20} />
                                    </a>
                                )}
            ...
            <div className="container mx-auto px-4 mt-12 pt-8 border-t border-slate-800 text-center text-sm text-slate-500">
                <p>&copy; {new Date().getFullYear()} {settings?.site_name || "دليل السويس"}. جميع الحقوق محفوظة.</p>
            </div>
        </footer>
    )
}
